/* ══════════════════════════════════════════
   MOTOR INTERACTIVO UNIFICADO DE LA PLATAFORMA
   ══════════════════════════════════════════ */
let curModIdx = null;
let curSesIdx = null;
let curCSEIdx = null;
    ...
    <button class="mod-card" onclick="openModulo(${i})">
      ...
      <div style="flex:1; min-width:0">
        ...
        <div class="mod-progress">
          ...
          <span class="mod-progress-txt">${done}/${total} sesion(es)</span>
...
function openModulo(idx) {
  ...
  document.getElementById('ses-count-label').textContent = `Estructura Pedagógica — ${m.sesiones.length} Sesión(es)`;
  ...
  document.getElementById('sesiones-list').innerHTML = m.sesiones.map((s, i) => {
    ...
    <button class="ses-card" onclick="openSesion(${i})" style="${done ? 'border-color:#6EE7B7; background:#F0FDF4' : ''}">
      ...
      <div class="ses-title" style="margin-top:4px">${s.title}</div>
      <div class="ses-obj" style="margin-top:2px">${s.obj[0]}</div>
    </button>`;
  }).join('');
...
function openSesion(idx) {
  ...
  btnNext.textContent = idx < m.sesiones.length - 1 ? 'Siguiente Sesión →' : '✓ Finalizar Módulo';
...
function updateDoneBtn() {
  ...
  txt.textContent = done ? 'Sesión Completada ✓ — Clic para revertir' : 'Marcar sesión como completada';
}
